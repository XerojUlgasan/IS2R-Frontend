...
export function getStocks(businessId, params = {}, opts = {}) {
  ...
  return apiRequest(`/api/businesses/${businessId}/stocks${query ? `?${query}` : ""}`, opts);
}
...
export function getStockHistory(businessId, stockId, params = {}, opts = {}) {
  const qs = new URLSearchParams();
  if (params.page) qs.set("page", params.page);
  if (params.limit) qs.set("limit", params.limit);
  const query = qs.toString();
