...
// Confirmation dialog for deleting a stock batch.
// Only available within 24 hours of creation.
function DeleteStockDialog({ stock, businessId, onClose, onDeleted, onUnauthorized }) {
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await deleteStock(businessId, stock.id);
      onDeleted(stock.id);
    } catch (err) {
      ...
      >
        <div className="flex items-start gap-md">
          ...
          <div className="flex flex-col gap-xs">
            <h2 className="font-headline-lg text-headline-md text-on-surface uppercase tracking-tight">Delete Stock</h2>
            <p className="font-body-sm text-body-sm text-on-surface-variant">
              Delete the stock batch for <span className="font-bold text-on-surface">{materialName}</span>?
              This will restore the consumed quantities. This action cannot be undone.
            </p>
          </div>
        </div>
