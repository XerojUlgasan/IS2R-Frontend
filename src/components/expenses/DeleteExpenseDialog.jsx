...
// Confirmation dialog for deleting an expense entry.
function DeleteExpenseDialog({ expense, onClose, onDeleted, onUnauthorized }) {
  ...
  const label = expense.title || "this expense";
      ...
      >
        <div className="flex items-start gap-md">
          ...
          <div className="flex flex-col gap-xs">
            <h2 className="font-headline-lg text-headline-md text-on-surface uppercase tracking-tight">Delete Expense</h2>
            <p className="font-body-sm text-body-sm text-on-surface-variant">
              Delete the expense <span className="font-bold text-on-surface">{label}</span>? This action cannot be undone.
            </p>
          </div>
        </div>
