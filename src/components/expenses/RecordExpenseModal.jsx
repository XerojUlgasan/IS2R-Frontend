...
// Modal for recording an expense. Fields: title, category, amount, remarks.
function RecordExpenseModal({ businessId, onClose, onSaved, onUnauthorized }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const data = await createExpense(businessId, {
      ...
      >
        <div className="flex items-start justify-between">
          <div className="flex flex-col gap-xs">
            <h2 className="font-headline-lg text-headline-md text-on-surface uppercase tracking-tight">Record Expense</h2>
            <p className="font-body-sm text-body-sm text-on-surface-variant">Log a business expense and its charge.</p>
          </div>
          <button
            onClick={onClose}
