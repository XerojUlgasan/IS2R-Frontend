...
import { useUpdateMaterial } from "../../hooks/useMaterialMutations";
...
// Modal to edit a material's name, type, and unit.
function EditMaterialModal({ material, onClose, onUpdated, onUnauthorized }) {
  const { update, submitting, error, setError } = useUpdateMaterial();
  const [name, setName] = useState(material.name || "");
  const [type, setType] = useState(material.type || MATERIAL_TYPES[0].value);
  const [unit, setUnit] = useState(material.unit || MATERIAL_UNITS[0].value);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updated = await update(material.id, { name: name.trim(), type, unit });
      onUpdated(updated);
    } catch (err) {
  ...
  return (
    <Modal title="Edit Material" subtitle="Update the material's details." onClose={onClose}>
